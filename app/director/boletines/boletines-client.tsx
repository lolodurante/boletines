...
export interface ReportCardListData {
...
interface Props {
  initialData: ReportCardListData
}

export function BoletinesClient({ initialData }: Props) {
  const searchParams = useSearchParams()
  const [courses] = useState(initialData.courses)
  const [periods] = useState(initialData.periods)
  const [reportCards, setReportCards] = useState(initialData.reportCards)
  ...
  const filteredReportCards = reportCards.filter(
    (r) => selectedCourseFilter === "all" || r.courseId === selectedCourseFilter,
  )
  ...
  const selectedReportMeta = reportCards.find((r) => r.id === selectedReportId)
  ...
  // Handle deep-link via ?report= or ?student= query params
  useEffect(() => {
    const reportId = searchParams.get("report")
    const studentId = searchParams.get("student")
    const requestedReport = reportId
      ? reportCards.find((report) => report.id === reportId)
      : studentId
      ? reportCards.find((report) => report.studentId === studentId)
      : undefined
    ...
    if (!selectedReportId || !reportCards.some((report) => report.id === selectedReportId)) {
      setSelectedReportId(reportCards[0]?.id)
    }
  }, [reportCards, searchParams, selectedReportId])

  // Fetch report detail on demand (lazy)
  useEffect(() => {
  ...
  // Fetch new list when period changes (server-side filtered)
  const handlePeriodChange = async (periodId: string) => {
    setSelectedPeriodFilter(periodId)
    setSelectedCourseFilter("all")
    setIsListLoading(true)
    const url =
      periodId === "all"
        ? "/api/director/report-cards"
        : `/api/director/report-cards?periodId=${periodId}`
    const response = await fetch(url, { cache: "no-store", credentials: "same-origin" })
    if (response.ok) {
      const data = (await response.json()) as ReportCardListData
      setReportCards(data.reportCards)
      setReportDetails({})
      setSelectedReportId(data.reportCards[0]?.id)
    } else {
          ...
          <Card className="flex flex-col min-h-0 h-full">
            <CardHeader className="shrink-0 pb-3">
              <div>
                ...
                <CardDescription>
                  {isListLoading ? "Cargando boletines..." : `${filteredReportCards.length} de ${reportCards.length} boletines`}
                </CardDescription>
              </div>
              <div className="grid grid-cols-1 gap-2">
                <Select value={selectedPeriodFilter} onValueChange={handlePeriodChange}>
